...
export function AnalysisProgress({
  ...
  const currentStage =
    stages.find((stage) => progress <= stage.threshold) || stages[0];
  ...
  return (
    <div className="space-y-6">
      <motion.div
        ...
        className="border border-neutral-800 hover:border-neutral-700 transition-all p-6 rounded-lg"
      >
        <div className="flex items-start gap-3">
          <Icon className="w-5 h-5 mt-0.5 text-green-400 animate-pulse" />
          <div className="space-y-2">
            <p className="text-sm font-medium">{currentStage.message}</p>
            <p className="text-xs text-neutral-200">{currentStage.detail}</p>
          </div>
        </div>
      </motion.div>
      ...
      {waitingForInput && progress === 80 && (
        <StrategyConfirmation onConfirm={() => onUserInput()} />
      )}
